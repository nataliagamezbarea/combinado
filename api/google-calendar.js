...
function formatAllDayDates(start, end) {
  ...
  if (endDate) {
    ...
    d.setDate(d.getDate() - 1); // end.date es exclusivo -> restamos 1 día
...
// --- Endpoint GET para crear un watch ---
...
// --- Webhook que recibe notificaciones de cambios ---
router.post("/webhook", async (req, res) => {
  try {
    ...
    const events = await calendar.events.list({
      ...
      singleEvents: true,
      showDeleted: true,
      updatedMin: twoMinutesAgo, // solo últimos 2 minutos
      maxResults: 20,
    });
    ...
    for (const ev of events.data.items || []) {
      ...
      try {
        ...
        if (!notionPageId) {
          ...
          // Actualizar extendedProperties origin si no está puesto
          await calendar.events.patch({
            calendarId: CALENDAR_ID,
            eventId: ev.id,
            requestBody: {
